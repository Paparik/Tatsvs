...
export const wellobject = {
    setup(){
        ...
        const wellImg = computed(() => {
            if(store.wellObj.imgWell.reader.path != null && store.wellObj.imgWell.reader.path.includes("./php")){
                return store.wellObj.imgWell.reader.path + "&csrf_token=" + document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            }
            else if(store.wellObj.imgWell.reader.path != null && !store.wellObj.imgWell.reader.path.includes("./php")){
                return store.wellObj.imgWell.reader.path;
            }
        })

        const wellSchemImg = computed(() => {
            if( store.wellObj.imgWellSchem.reader.path != null &&  store.wellObj.imgWellSchem.reader.path.includes("./php")){
                return  store.wellObj.imgWellSchem.reader.path + "&csrf_token=" + document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            }
            else if( store.wellObj.imgWellSchem.reader.path != null && !store.wellObj.imgWellSchem.reader.path.includes("./php")){
                return  store.wellObj.imgWellSchem.reader.path;
            }
        })
    ...
    template: `
        <div class="items">
            <div class="mainkk-input">
                <input readonly v-model="store.wellObj.schemaName" style="background: #fff;box-shadow:0px 8px 22px 1px rgba(0, 0, 0, 0.25); text-align: center; padding-left: 0;" type="text" placeholder="Название схемы">
            </div>
                ...
                <div class="item__content">
                    <div class="table">
                        ...
                        <div class="table__item" v-if="!typeWell">
                            ...
                            <div class="table__content" v-if="store.wellObj.imgWell.reader">
                                <div class="item-img">
                                    <img :src="wellImg" alt="">
                                </div>
                            </div>
                        ...
                        <div class="table__item" v-if="!typeWell">
                            ...
                            <div class="table__content " v-if="store.wellObj.imgWellSchem.reader">
                                <div class="item-img">
